...
function restart_miner() {
    docker stop miner && docker rm miner && bash $SCRIPT_PATH/miner.sh
    echo "$CONTAINER_NAME was restarted" >> $LOG_FILE
}

if [ `docker ps --quiet --filter "name=$CONTAINER_NAME" | wc -l` -eq 0 ]; then
    # restart miner if it is not present
    restart_miner
    echo "container has been stopped with some reason" >> $LOG_FILE
elif [ `docker logs miner | grep -i SIGSEGV | wc -l` -gt 0 ]; then
